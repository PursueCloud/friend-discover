 ...
 * 
 */
package com.yo.friendis.core.hadoop.mapred.fastcluster;

import com.yo.friendis.core.hadoop.mapred.fastcluster.mr.ClusterDataMapper;
...
public class ClusterDataJob extends Configured implements Tool {
	...
	public int run(String[] args) throws Exception {
	    ...
	    // 把已经分类的个数和未分类的个数赋值出去
	    HadoopUtils.CLUSTERED=job.getCounters().findCounter(ClusterCounter.CLUSTERED).getValue();
	    HadoopUtils.UNCLUSTERED=job.getCounters().findCounter(ClusterCounter.UNCLUSTERED).getValue();
	    return ret;
	}
