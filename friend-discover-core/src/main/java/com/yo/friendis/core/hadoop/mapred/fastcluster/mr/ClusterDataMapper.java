...
import com.yo.friendis.core.hadoop.mapred.fastcluster.ClusterCounter;
	...
	public void map(IntWritable key,DoubleArrIntWritable  value,Context cxt){
		...
		try {
			...
			for(int i=start;i<iter_i;i++){// all files are clustered points
				...
				parentFolder= HadoopUtils.CENTERPATH+"/iter_"+i+"/clustered";
				...
				while(files.hasNext()){
					...
					while (reader.next(dkey, dvalue)) {// read file literally
						distance = HadoopUtils.getDistance(inputI, dvalue.getDoubleArr());
