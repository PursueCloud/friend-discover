...
import com.yo.friendis.core.hadoop.mapred.fastcluster.keytype.CustomDoubleWritable;
...
import com.yo.friendis.core.hadoop.model.IDistanceDensityMul;
...
//		String path = "hdfs://master:9000/user/root/iris_deltadistance/part-r-00000";
//		drawPic(path,null);
//	}
...
//				list.add(dvalue.getSum()*dvalue.getDistance());
	...
	private static List<IDistanceDensityMul> getIDistanceDensityMulList(String url) throws FileNotFoundException, IOException{
		...
		FileStatus[] fss=HadoopUtils.getHDFSPath(url,"true").getFileSystem(conf)
				.listStatus(HadoopUtils.getHDFSPath(url, "true"));
...
//				 	DoubleWritable,  IntDoublePairWritable
				...
				IntDoublePairWritable dvalue = (IntDoublePairWritable) ReflectionUtils.newInstance(
						reader.getValueClass(), conf);
				int i= CommonUtils.GETDRAWPICRECORDS_EVERYFILE;
