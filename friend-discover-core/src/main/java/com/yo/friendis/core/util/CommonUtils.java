package com.yo.friendis.core.util;

import com.yo.friendis.core.hadoop.model.HadoopCfgConst;
import com.yo.friendis.core.hadoop.model.ObjectInterface;
...
public class CommonUtils {
    private static HadoopCfgConstService haCfgConstService;
    ...
    public CommonUtils(HadoopCfgConstService haCfgConstService) {
        CommonUtils.haCfgConstService = haCfgConstService;
    }
    /**
     * 初始化登录表数据
     */
...
//	}
    ...
    public static String getKey(String key,boolean dbOrFile){
        if(dbOrFile){
            List<HadoopCfgConst> haCfgConstList = haCfgConstService.selectAllByEqField(HadoopCfgConst.FIELD_CONSTKEY, key);
            if( haCfgConstList!=null && haCfgConstList.size()>0 ) {
                return haCfgConstList.get(0)!=null ? haCfgConstList.get(0).getConstValue().toString() : "";
            } else {
                return "";
            }
        }
    ...
    public static boolean changeDat2Xml(String xmlFolder){
        ...
        try{
            ...
            for(File f:files){
                ...
                if(filename.lastIndexOf(HadoopUtils.DOWNLOAD_EXTENSION)!=-1){// 匹配文件,则进行转换
                    newFilename=filename.substring(0, filename.lastIndexOf(HadoopUtils.DOWNLOAD_EXTENSION))+".xml";
     ...
     * @throws IOException
     */
    public static List<String[]> parseDat2StrArr(String datFile) throws IOException{
        List<String[]> list = new ArrayList<String[]>();
        ...
        while((line = br.readLine()) != null) {
            ...
            for(int i=1;i<userdata_attributes.length;i++){
                arr[i]=getAttrValInLine(line, userdata_attributes[i]);
            }
            list.add(arr);
        }
        //去重第1,2,3和最后一行
    ...
    /**
     * 获取给定数组两两之间的距离(欧式距离:HadoopUtils.getDistance() )，每个行为一个向量
     * 并把这些距离按照大小从小到大排序
     * @param vectors
     * @return
     */
    public static double[] getDistances(double[][] vectors) {
        ...
        for(int i=0;i<vectors.length;i++){
            for(int j=i+1;j<vectors.length;j++){
                distances[k++]=HadoopUtils.getDistance(vectors[i], vectors[j]);
            }
        }
        // sort
     ...
     * @throws IOException
     */
    public static List<String> getLines(String input, boolean isAbsolutePath) throws IOException {
        List<String> list = new ArrayList<String>();
        if( !isAbsolutePath ) {
            input = getRootPathBasedPath(input);
        }
     ...
     * @return
     */
    public static String object2Percent(Object dou,int dotNum){
